...
// Helper to run ephemeral HTTP server with customized app
...
test(
  "POST /api/v1/agents: valid agent creation returns HTTP 201 with generated UUID and data",
  async () => {
    const mockAgent = {
...
test(
  "POST /api/v1/agents: applies defaults when only name is provided",
  async () => {
    let capturedInput;
...
test(
  "POST /api/v1/agents: missing name returns HTTP 400 with INVALID_AGENT",
  async () => {
    const fakeRepo = {
      async createAgent() {},
    };
...
test(
  "POST /api/v1/agents: empty name string returns HTTP 400 with INVALID_AGENT",
  async () => {
    const fakeRepo = {
      async createAgent() {},
    };
...
test(
  "POST /api/v1/agents: invalid status returns HTTP 400 with INVALID_AGENT",
  async () => {
    const fakeRepo = {
      async createAgent() {},
    };
...
test(
  "POST /api/v1/agents: invalid environment returns HTTP 400 with INVALID_AGENT",
  async () => {
    const fakeRepo = {
      async createAgent() {},
    };
...
test(
  "POST /api/v1/agents: malformed JSON returns HTTP 400 with INVALID_JSON",
  async () => {
    const fakeRepo = {
      async createAgent() {},
    };
...
test(
  "POST /api/v1/agents: repository/database failure returns HTTP 500 without leaking raw DB error",
  async () => {
    const fakeRepo = {
      async createAgent() {
...
test(
  "GET /api/v1/agents/:id: valid existing agent returns HTTP 200 with agent record",
  async () => {
    const mockAgent = {
      id: "3fa85f64-5717-4562-b3fc-2c963f66afa6",
    ...
    await withCustomServer({ agentRepository: fakeRepo }, async (baseUrl) => {
      const res = await fetch(
        `${baseUrl}/api/v1/agents/${mockAgent.id}`
      );
...
test(
  ...
  async () => {
    ...
    await withCustomServer({ agentRepository: fakeRepo }, async (baseUrl) => {
      const res = await fetch(
        `${baseUrl}/api/v1/agents/${nonExistentUuid}`
      );
...
test(
  "GET /api/v1/agents/:id: invalid UUID format returns HTTP 400 with INVALID_AGENT_ID",
  async () => {
    const fakeRepo = {
      async getAgentById() {},
    };
    ...
    await withCustomServer({ agentRepository: fakeRepo }, async (baseUrl) => {
      const res = await fetch(
        `${baseUrl}/api/v1/agents/not-a-uuid-12345`
      );
...
test(
  "GET /api/v1/agents/:id: repository/database failure returns HTTP 500 without leaking error",
  async () => {
    const fakeRepo = {
      async getAgentById() {
    ...
    await withCustomServer({ agentRepository: fakeRepo }, async (baseUrl) => {
      const res = await fetch(
        `${baseUrl}/api/v1/agents/${validUuid}`
      );
      ...
      assert.equal(
        body.error.message,
        "Failed to retrieve agent record"
      );
    });
  }
);
...
test(
  "GET /api/v1/agents: successful list returns HTTP 200 with array of agents",
  async () => {
    const mockAgents = [
      {
        id: "3fa85f64-5717-4562-b3fc-2c963f66afa6",
...
test(
  "GET /api/v1/agents: pagination returns default page=1, limit=20 and pagination metadata",
  async () => {
...
test(
  "GET /api/v1/agents: pagination respects custom page and limit query params",
  async () => {
    let receivedOptions = null;
    ...
    await withCustomServer({ agentRepository: fakeRepo }, async (baseUrl) => {
      const res = await fetch(
        `${baseUrl}/api/v1/agents?page=2&limit=50`
      );
...
test(
  "GET /api/v1/agents: rejects invalid pagination params with HTTP 400 INVALID_PAGINATION",
  async () => {
    const fakeRepo = {
      async listAgents() {},
    };
...
test(
  "Offline mode: when Supabase is not configured, endpoints return HTTP 503 SUPABASE_NOT_CONFIGURED",
  async () => {
    await withServer(async (baseUrl) => {
      ...
      assert.equal(
        getListBody.error.code,
        "SUPABASE_NOT_CONFIGURED"
      );

      const getOneRes = await fetch(
        `${baseUrl}/api/v1/agents/3fa85f64-5717-4562-b3fc-2c963f66afa6`
      );
